...
@interface UITabBar (GlossyGlassChrome)
- (void)gg_chrome_layoutSubviews;
@end
@implementation UITabBar (GlossyGlassChrome)
- (void)gg_chrome_layoutSubviews {
    [self gg_chrome_layoutSubviews];
    @try {
        Class shell = NSClassFromString(@"GlassChromeShell");
        if (shell && [shell respondsToSelector:@selector(shared)]) {
            id s = ((id (*)(Class, SEL))objc_msgSend)(shell, @selector(shared));
            if ([s respondsToSelector:@selector(paintIfChrome:)]) {
                ((void (*)(id, SEL, id))objc_msgSend)(s, @selector(paintIfChrome:), self);
            }
        }
...
@interface UINavigationBar (GlossyGlassChrome)
- (void)gg_chrome_layoutSubviews;
@end
@implementation UINavigationBar (GlossyGlassChrome)
- (void)gg_chrome_layoutSubviews {
    [self gg_chrome_layoutSubviews];
    @try {
        Class shell = NSClassFromString(@"GlassChromeShell");
...
static void GG_SafeApply(void) {
    @try {
        Class bus = NSClassFromString(@"GlassSyncBus");
        if (bus && [bus respondsToSelector:@selector(shared)]) {
            id shared = ((id (*)(Class, SEL))objc_msgSend)(bus, @selector(shared));
            if ([shared respondsToSelector:@selector(publishHierarchyChanged)]) {
                ((void (*)(id, SEL))objc_msgSend)(shared, @selector(publishHierarchyChanged));
                return;
            }
        }
        ...
        Class shell = NSClassFromString(@"GlassChromeShell");
        if (shell && [shell respondsToSelector:@selector(shared)]) {
            id shared = ((id (*)(Class, SEL))objc_msgSend)(shell, @selector(shared));
            if ([shared respondsToSelector:@selector(reassert)]) {
                ((void (*)(id, SEL))objc_msgSend)(shared, @selector(reassert));
            }
        }
    } @catch (__unused NSException *e) {}
}
...
static void GG_InstallHooks(void) {
    GG_SafeSwizzleInstance([UITabBar class], @selector(layoutSubviews), @selector(gg_chrome_layoutSubviews));
    GG_SafeSwizzleInstance([UINavigationBar class], @selector(layoutSubviews), @selector(gg_chrome_layoutSubviews));
    // Late IG classes
    Class igTab = NSClassFromString(@"IGTabBar");
    if (igTab) GG_SafeSwizzleInstance(igTab, @selector(layoutSubviews), @selector(gg_chrome_layoutSubviews));
    Class igNav = NSClassFromString(@"IGNavigationBar");
    if (igNav) GG_SafeSwizzleInstance(igNav, @selector(layoutSubviews), @selector(gg_chrome_layoutSubviews));
