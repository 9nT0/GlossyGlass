...
@objc public class GlassDiagnostics: NSObject {
    ...
    @objc public private(set) var chromeSurfaces: Int = 0
    @objc public private(set) var chromeNote: String = ""
    @objc public private(set) var chromeMap: String = ""
    ...
    @objc public func recordChrome(surfaces: Int, map: [String: Int], note: String) {
        chromeSurfaces = surfaces
        chromeMap = map.map { "\($0.key):\($0.value)" }.sorted().joined(separator: ",")
        chromeNote = note
    }
    ...
    @objc public func summary() -> String {
        ...
        SettingsAttached: \(isAttached)
        SettingsHost: \(lastHostClass)
        SettingsScore: \(lastScore)
        SettingsNote: \(lastInjectionNote)

        ChromeAttached: \(chromeSurfaces > 0)
        ChromeSurfaces: \(chromeSurfaces)
        ChromeMap: \(chromeMap.isEmpty ? "none" : chromeMap)
        ChromeNote: \(chromeNote.isEmpty ? "none" : chromeNote)
        ActiveSurface: \(GlassSurfaceRouter.shared.activeSurfaceName)
        ChromeCandidates: \(GlassLocator.shared.chromeCandidateCount())
        ChromeDetail: \(GlassLocator.shared.chromeCandidatesSummary())
