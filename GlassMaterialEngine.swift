...
@objc public final class GlassMaterialEngine: NSObject {
    ...
    private func installCompactPlate(into host: UIView, effect: UIBlurEffect, style: String, intensity: CGFloat, opacity: CGFloat) {
        ...
        let side: CGFloat = 14
        let h = min(54, max(46, b.height - 10))
        let y = max(4, (b.height - h) * 0.35)
        plate.frame = CGRect(x: side, y: y, width: b.width - side * 2, height: h)

        let radius = min(22, h * 0.42)
        plate.layer.cornerRadius = radius
        if #available(iOS 13.0, *) { plate.layer.cornerCurve = .continuous }
        plate.alpha = max(0.9, min(1, opacity))
    ...
    @objc public func updateSelectedLens(in host: UIView, index: Int, count: Int) {
        ...
        let f = CGRect(
            x: CGFloat(index) * w + w * 0.16,
            y: plate.bounds.height * 0.16,
            width: w * 0.68,
            height: plate.bounds.height * 0.68
        )
        lens.frame = f
        lens.layer.cornerRadius = min(14, f.height * 0.38)
    ...
    @objc public func neutralizeStockChrome(_ view: UIView) {
        ...
        for sub in view.subviews {
            if sub.tag == plateTag || sub.tag == tintTag { continue }
